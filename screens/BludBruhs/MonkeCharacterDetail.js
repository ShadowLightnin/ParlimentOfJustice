...
const MonkeCharacterDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchCharacterData = async () => {
      if (member?.id && (!member.mediaUri || !member.mediaType)) {
        try {
          const docRef = doc(db, 'powerMonkeMembers', member.id);
          const docSnap = await getDoc(docRef);
          if (docSnap.exists()) {
            const data = docSnap.data();
            console.log('Fetched character data from Firestore:', data);
            setName(data.name || '');
          ...
          console.error('Error fetching character data:', e.message, e.stack);
          Alert.alert('Error', `Failed to fetch character data: ${e.message}`);
        }
        ...
        console.log('Loading member data:', {
          ...
          mediaUri: member.mediaUri || member.videoUri,
    ...
    return () => {
      authUnsub();
      if (audioRef.current) {
        audioRef.current.unloadAsync().catch((e) => console.error('Audio unload on cleanup:', e));
  ...
  useEffect(() => {
    ...
    return () => {
      if (audioRef.current) {
        audioRef.current.unloadAsync().catch((e) => console.error('Audio unload on useEffect cleanup:', e));
        audioRef.current = null;
        console.log('Audio unloaded on useEffect cleanup');
      }
    };
  ...
  const hasChanges = () => {
    ...
    return (
      ...
      mediaUri !== (member.mediaUri || member.videoUri || null) ||
      mediaType !== (member.mediaType || (member.videoUri ? 'video' : null))
    );
  };
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      let mediaUrl = mediaUri && mediaUri.startsWith('http') ? mediaUri : member?.mediaUri || member?.videoUri || null;
      let savedMediaType = mediaType || member?.mediaType || (member?.videoUri ? 'video' : null);
      if (mediaUri && !mediaUri.startsWith('http')) {
        console.log(`Uploading media: ${mediaUri}`);
        mediaUrl = await uploadMedia(mediaUri, mediaType || 'media');
        if (member?.mediaUri || member?.videoUri) {
        ...
        console.log('Clearing media URI as no new media was selected');
        await deleteOldMedia(member.mediaUri || member.videoUri);
        mediaUrl = null;
        savedMediaType = null;
      }
      ...
      if (charId) {
        ...
        await setDoc(doc(db, 'powerMonkeMembers', charId), charData, { merge: true });
        ...
        const charRef = await addDoc(collection(db, 'powerMonkeMembers'), charData);
        charId = charRef.id;
  ...
  const handleBackPress = async () => {
    ...
    if (audioRef.current) {
      try {
        ...
        console.error('Audio stop error on back press:', e.message, e.stack);
      }
    }
    if (videoRef.current) {
      try {
    ...
    setIsPlaying(false);
    navigation.navigate('PowerMonke', {
      isPlaying: false,
...
const styles = StyleSheet.create({
  ...
  backButtonText: { fontSize: 24, color: '#804d00', fontWeight: 'bold' },
  ...
  title: { fontSize: 28, fontWeight: 'bold', color: '#df7345', textAlign: 'center', flex: 1, textShadowColor: '#805300', textShadowOffset: { width: 1, height: 2 }, textShadowRadius: 10 },
  input: { backgroundColor: '#333', color: '#fff', padding: 10, borderRadius: 5, marginBottom: 10, fontSize: 16 },
  textArea: { height: 100, textAlignVertical: 'top' },
  button: { backgroundColor: '#805300', padding: 10, borderRadius: 5, alignItems: 'center', marginBottom: 10 },
  saveButton: { backgroundColor: '#805300', padding: 15, borderRadius: 5, alignItems: 'center' },
  ...
  clickable: (borderColor) => ({ borderWidth: 2, borderColor: borderColor || '#805300', shadowColor: borderColor || '#805300', shadowOpacity: 0.8, shadowRadius: 10 }),
  ...
  playButton: { backgroundColor: '#805300', padding: 5, borderRadius: 5, alignSelf: 'center', marginTop: 5 },
  ...
  closeButton: { backgroundColor: '#805300', padding: 10, borderRadius: 5, alignSelf: 'center' },
  ...
  modalCancel: { backgroundColor: '#805300', padding: 10, borderRadius: 5, flex: 1, marginRight: 10 },
...
export default MonkeCharacterDetail;