...
const StatuesScreen = () => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, 'statues'), (snap) => {
      ...
      dynamicStatues.forEach(s => {
        ...
        nameCounts[s.name || s.codename || 'Unknown'] = (nameCounts[s.name || s.codename || 'Unknown'] || 0) + 1;
      });
    ...
    try {
      ...
      if (imageUrl && imageUrl !== 'placeholder') {
        ...
        try {
          ...
          Alert.alert('Warning', `Failed to delete image from storage: ${e.message}. Statue will still be deleted.`);
        }
      } else {
    ...
    <View key={statue.id} style={styles.statueCont}>
      ...
      {statue.hardcoded === false && (
        <View style={styles.buttons}>
          ...
          <TouchableOpacity
            onPress={() => setDeleteModal({ visible: true, statue: { id: statue.id, name: statue.name || statue.codename || 'Unknown' } })}
  ...
  const renderPreviewCard = (statue) => (
    <TouchableOpacity
      style={[styles.previewCard(isDesktop, SCREEN_WIDTH), styles.clickable(statue.borderColor || '#c0c0c0')]}
      onPress={() => {
    ...
    >
      <View style={styles.overlay}>
          ...
          <View style={styles.modalBackground}>
            ...
            >
              ...
              <View style={styles.previewAboutSection}>
                <Text style={styles.previewName}>{previewStatue?.name || previewStatue?.codename || 'Unknown'}</Text>
                <Text style={styles.previewDesc}>{previewStatue?.description || 'No description available'}</Text>
                <TouchableOpacity
                  onPress={() => {
                    console.log('Closing preview modal');
                    setPreviewStatue(null);
        ...
        <Modal
          ...
          transparent
          animationType="slide"
          onRequestClose={() => setDeleteModal({ visible: false, statue: null })}
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              <Text style={styles.modalText}>{`Delete "${deleteModal.statue?.name || ''}" and its image?`}</Text>
              <View style={styles.modalButtons}>
                <TouchableOpacity
                  style={styles.modalCancel}
                  onPress={() => setDeleteModal({ visible: false, statue: null })}
                >
                  <Text style={styles.modalCancelText}>Cancel</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.modalDelete}
                  onPress={() => deleteModal.statue && confirmDelete(deleteModal.statue.id)}
                >
            ...
            <View style={styles.summonModalContainer}>
              <View style={styles.summonModalContent}>
                <Text style={styles.summonModalText}>
                  🔥 You have summoned: {selectedStatue?.name || selectedStatue?.codename || 'Unknown'} 🔥
                </Text>
              </View>
            </View>
          </TouchableWithoutFeedback>
