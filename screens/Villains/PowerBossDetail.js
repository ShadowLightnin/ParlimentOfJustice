...
const PowerBossDetail = () => {
  ...
  useEffect(() => {
      ...
      if (user) {
        try {
          ...
          console.log('Checking auth:', {
            ...
            isAllowed: RESTRICT_ACCESS ? ALLOWED_EMAILS.includes(user.email) : true,
            allowedEmails: ALLOWED_EMAILS,
          });
          const isAllowed = RESTRICT_ACCESS ? ALLOWED_EMAILS.includes(user.email) : true;
    ...
    const fetchCharacterData = async () => {
      if (member?.id) {
        try {
          const docRef = doc(db, 'powerBossMembers', member.id);
          const docSnap = await getDoc(docRef);
          if (docSnap.exists()) {
            const data = docSnap.data();
            console.log('Fetched Big Bad data from Firestore:', data);
            setName(data.name || '');
            ...
            console.warn('Big Bad not found in Firestore:', member.id);
          }
        } catch (e) {
          console.error('Error fetching Big Bad data:', e.message, e.stack);
          Alert.alert('Error', `Failed to fetch Big Bad data: ${e.message}`);
        }
  ...
  const uploadMedia = async (uri, type) => {
    ...
    try {
      ...
      const path = `bigBads/${timestamp}_${random}.${extension}`;
  ...
  const saveChanges = async () => {
    ...
    if (!name.trim()) {
      ...
      Alert.alert('Error', 'Please provide a Big Bad name.');
      return false;
    }
    if (!auth.currentUser) {
    ...
    try {
      ...
      console.log('Saving Big Bad data:', charData);
      let charId = member?.id;
      if (charId) {
        console.log(`Updating Big Bad with ID: ${charId}`);
        await setDoc(doc(db, 'powerBossMembers', charId), charData, { merge: true });
        console.log('Big Bad updated:', { id: charId, name: charData.name, images: charData.images.length, mediaUri: charData.mediaUri });
      } else {
        console.log('Adding new Big Bad');
        const charRef = await addDoc(collection(db, 'powerBossMembers'), charData);
        charId = charRef.id;
        console.log('Big Bad added:', { id: charId, name: charData.name, images: charData.images.length, mediaUri: charData.mediaUri });
      }
      Alert.alert('Success', `Big Bad ${charId ? 'updated' : 'added'} successfully!`);
      setMode('view');
      setImages(imageUrls);
      ...
      Alert.alert('Error', `Failed to save Big Bad: ${e.message}`);
      return false;
    } finally {
      setUploading(false);
    }
    ...
    >
      <Image
        ...
        onError={(e) => {
          ...
          setPreviewImage({ ...img, uri: PLACEHOLDER_IMAGE });
        }}
      />
      {img.name ? (
        <View style={styles.imageNameContainer}>
      ...
      <View style={styles.overlay}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          <View style={styles.headerContainer}>
            <Text style={styles.title}>{name || 'Add/Edit Big Bad'}</Text>
          </View>
          {mode !== 'view' && (
            <>
              <TextInput style={styles.input} placeholder="Big Bad Name" value={name} onChangeText={setName} editable={canSubmit} />
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 28, fontWeight: 'bold', color: '#630404', textAlign: 'center', flex: 1, textShadowColor: '#9561f5', textShadowOffset: { width: 1, height: 2 }, textShadowRadius: 10 },
...
export default PowerBossDetail;