// screens/Planets/PinnaclePlanets.js
...
// Only show these systems in Pinnacle
const PINNACLE_SYSTEM_IDS = ['sol', 'ignis', 'zaxxon'];

// Exclude hypothetical Sol objects in Pinnacle view
const PINNACLE_EXCLUDED_IDS = ['planet9', 'planet10'];

const PinnaclePlanets = () => {
  ...
  const currentUniverseLabel = 'Pinnacle Universe';
  ...
  useEffect(() => {
    const loadUniverse = async () => {
      try {
        // Start from global sorted list
        let sorted = sortPlanets();

        // Limit to Sol / Ignis / Zaxxon
        sorted = sorted.filter(
          (p) =>
            PINNACLE_SYSTEM_IDS.includes(p.systemId) &&
            !PINNACLE_EXCLUDED_IDS.includes(p.id)
        );
        ...
        if (initialPlanetIdFromRoute) {
          ...
          // Default to Melcornia in Pinnacle, if present
          const defaultId = 'melcornia';
        ...
        console.error('Error loading pinnacle planets', e);
        const sorted = sortPlanets().filter(
          (p) =>
            PINNACLE_SYSTEM_IDS.includes(p.systemId) &&
            !PINNACLE_EXCLUDED_IDS.includes(p.id)
        );
  ...
  if (!currentPlanet) {
    return (
      <View style={styles.loadingContainer}>
        <ActivityIndicator size="large" color="#ff6bd9" />
        <Text style={styles.loadingText}>Loading pinnacle system…</Text>
      </View>
    );
  }

  // FILTER SYSTEMS FOR PINNACLE
  const systemsForUniverse = SYSTEMS.filter((s) =>
    PINNACLE_SYSTEM_IDS.includes(s.id)
  );
  ...
  const openGalaxyMap = () => {
    ...
    setTimeout(() => {
      // Use a dedicated Pinnacle galaxy map route
      navigation.navigate('PinnacleGalaxyMap', {
        fromUniverse: 'pinnacle',
        ...
        <View style={styles.overlay}>
          ...
          <View style={styles.header}>
            ...
            <TouchableOpacity
              ...
              activeOpacity={0.85}
            >
              <Text style={styles.galaxyButtonText}>Pinnacle Galaxy ✨</Text>
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  backButton: {
    paddingVertical: 8,
    paddingHorizontal: 12,
    backgroundColor: 'rgba(60, 0, 40, 0.7)',
    borderRadius: 999,
    borderWidth: 1,
  ...
  galaxyButton: {
    ...
    borderRadius: 999,
    backgroundColor: 'rgba(40, 0, 70, 0.9)',
    borderWidth: 1,
  ...
  bottomHud: {
    ...
    paddingVertical: 8,
    borderTopWidth: 1,
    borderTopColor: 'rgba(255, 80, 160, 0.45)',
    backgroundColor: 'rgba(10, 0, 25, 0.96)',
  },
  bottomDivider: {
    height: 1,
    backgroundColor: 'rgba(200, 80, 255, 0.5)',
    marginVertical: 4,
  },
  ...
  systemButton: {
    ...
    borderRadius: 999,
    borderWidth: 1,
    borderColor: 'rgba(220, 90, 255, 0.85)',
    backgroundColor: 'rgba(25, 0, 45, 0.95)',
    alignItems: 'center',
  },
  ...
  bottomNameHint: {
    fontSize: 9,
    color: 'rgba(255, 210, 245, 0.8)',
  },
  bottomUniverse: {
    flex: 1,
    fontSize: 11,
    color: 'rgba(245, 190, 255, 0.9)',
    fontStyle: 'italic',
    textAlign: 'right',
  },
  ...
  earthToggleButton: {
    ...
    borderRadius: 999,
    borderWidth: 1,
    borderColor: 'rgba(230, 110, 255, 0.7)',
    backgroundColor: 'rgba(30, 0, 55, 0.85)',
  },
  earthToggleButtonActive: {
    backgroundColor: 'rgba(255, 70, 140, 0.95)',
    borderColor: 'rgba(255, 230, 255, 0.95)',
  },
  earthToggleText: {
    fontSize: 11,
    color: 'rgba(255, 220, 250, 0.9)',
    fontWeight: '500',
  },
  ...
  planetNavButton: {
    ...
    borderWidth: 1,
    borderColor: 'rgba(220, 90, 255, 0.85)',
    backgroundColor: 'rgba(25, 0, 50, 0.95)',
    alignItems: 'center',
  },
  ...
  infoPanelCard: {
    ...
    borderRadius: 18,
    borderWidth: 1,
    borderColor: 'rgba(230, 120, 255, 0.85)',
    backgroundColor: 'rgba(10, 0, 30, 0.98)',
    paddingHorizontal: 12,
    paddingVertical: 8,
  },
  infoRowTop: {
  ...
  infoTag: {
    ...
    borderRadius: 999,
    borderWidth: 1,
    borderColor: 'rgba(235, 140, 255, 0.8)',
  },
  infoScroll: {
  ...
  infoCloseButton: {
    ...
    borderRadius: 999,
    borderWidth: 1,
    borderColor: 'rgba(230, 130, 255, 0.9)',
    backgroundColor: 'rgba(25, 0, 55, 0.96)',
  },
  infoCloseText: {
    fontSize: 13,
    fontWeight: 'bold',
...
export default PinnaclePlanets;
